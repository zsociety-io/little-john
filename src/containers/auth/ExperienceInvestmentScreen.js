...
export default ExperienceInvestmentScreen = ({navigation}) => {
  const colors = useSelector(state => state.theme.theme);
  const [isSelected, setIsSelected] = useState('');

  const onPressItem = val => setIsSelected(val);

  const onPressContinue = () => navigation.navigate(StackNav.WorkTypeScreen);
  ...
  const RenderHeaderComponent = () => (
    <CText type={'B30'}>{strings.question6}</CText>
  );
